...
function getTime(startStr, endStr) {
   // Get the current date
   const now = new Date();

   const start = new Date(startStr || now);
   if (!startStr) {
      start.setMonth(now.getMonth() - 1);
      start.setHours(0, 0, 0, 0);
   }

   const end = new Date(endStr || now);
...
function login(cred) {
   ...
   const {deviceId, timeStamp, sessionId} = prepareOASign('wms.member.login', params);
   const data = `api=wms.member.login&version=${apiVersion}&timestamp=${timeStamp}&params=${encodeURIComponent(params)}&`
   api = 'https://wms-api.flashfulfillment.vn/?api=api=wms.member.login';
   headers = {
      ...
      'Origin': 'https://wms.flashfulfillment.vn',
...
async function getDOs(DOs, start, end) {
   try {
      const promises = [];
      for (const DO of DOs) {
         const rs = await getDO(DO, start, end);
         promises.push(rs);
      ...
      const result = promises.map(item => {
         ...
         if (!item.success) {
            if (item.errorCode == 'AD.SessionNotExists' || item.errorMessage == 'YOUR_ACCOUNT_IS_ALREADY_LOGGED_IN_FROM_ANOTHER_LOCATION') {
...
function getDODetails(DO, start, end) {
   return getDO(DO, start, end).then(DOInfo => {
      if (!DOInfo.success) {
         if (DOInfo.errorCode == 'AD.SessionNotExists' || DOInfo.errorMessage == 'YOUR_ACCOUNT_IS_ALREADY_LOGGED_IN_FROM_ANOTHER_LOCATION') {
            throw new Error("Session expired", {cause: {code: "SESSIONEXPIRED", message: "Session expired" }});
         }
      }
      ...
      const params2 = `{"deliveryOrderId":"${id}"}`;
      ({deviceId, timeStamp, sessionId} = prepareOASign('wms.occupyInventoryLog.listByDeliveryOrderId', params2));
      const data2 = `api=wms.occupyInventoryLog.listByDeliveryOrderId&version=1.0&timestamp=${timeStamp}&params=${encodeURIComponent(params2)}&`
      const api2 = 'https://wms-api.flashfulfillment.vn/?api=wms.occupyInventoryLog.listByDeliveryOrderId';
      headers2 = {
         'Accept': 'application/json, text/plain, */*',
         'Content-Type': 'application/x-www-form-urlencoded',
         'OA-App-Key': '1001520',
...
async function getDOsDetails(DOs, start, end) {
   try {
      ...
      console.log('Requests fulfiled: ', promises.length);
      let failed = 0, notFound = 0;
      const result = promises.map(item => {
         ...
         if (!doDetails.success || !routeDetails.success || !occupyInventoryLog.success) {
            if (item.errorCode == 'AD.SessionNotExists') {
               throw new Error("Session expired", {cause: {code: "SESSIONEXPIRED", message: "Session expired" }});
            }
         }
         if (doDetails.data && routeDetails.data && occupyInventoryLog.data) {
            obj = doDetails.data;
